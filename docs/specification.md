...
- **`GetSystemPowerStatus` 失败就跳过这一拍**：它失败时的返回与「100%、使用电池」这个真实读数
  长得一模一样，当成真值会让托盘跳 `FL`、清空「最近实测」，还往日志里写下一条从未发生过的步进
  和一行状态行。没读到就是没读到 —— 不刷图标、不更新 tooltip、不写日志、不入环形缓冲，
  等下一个电源事件。启动/`TaskbarCreated`/DPI 变化那几次强制刷新同理，图标就先保持原样。
- 启动时先主动刷新一次作为初始状态。
...
- **例外：信息面板可见期间**（见 [2.10](#210-电池信息面板)）用 `SetTimer` 每 2 秒重采一次功率与温度，
  ...
  - `KillTimer` 必须在**所有**隐藏路径上都走到（失焦隐藏、Esc、再次单击）。
...
- **留边距，不顶满**：拟合时只用图标边长的 **90%** 作为宽高预算 —— 顶满整个方框的数字比系统自己的托盘图标显眼一大截，而按大字号画好位图再交给 shell 缩小（`percentage` 那种做法）又偏小；90% 是两者之间。再调低会让笔画变细发虚（且不能用加粗补，见上条）。最终生效的只有拟合出的字号，而它按超采样倍数跳档，所以一段区间的比例值会得到同一结果 —— 反过来说，改动两三个百分点可能毫无变化，也可能整档掉下去，取决于当前 DPI 与字面的度量。**改完必须在实机的托盘里看一眼**，这个数是靠并排比出来的，不是算出来的。
...
- **拟合不出字号就不出图标**（字体建不出来，或最小一档也放不下）：托盘保留上一个图标，
  绝不拿旧 DPI 的字号和基线往新尺寸的位图上画 —— 那画出来是错位或被裁掉的字，比不更新更糟。
  字号、基线与位图尺寸是一组状态，拟合失败时必须整组失效。
...
- 这一次读取是全程序唯一的主题判断：托盘图标文字色、信息面板配色（[2.10](#210-电池信息面板)）
  与右键菜单（[2.7](#27-右键上下文菜单)）都取自它，不各读各的，也不掺 `AppsUseLightTheme`。

### 2.5 变化去重

电源事件可能短时间内重复触发。仅当 `(percentage, chargeState)` 相对上次**确有变化**时才重建图标与写日志（沿用缓存判断作为一层防抖），避免无谓的图标重建。

- 判据同一条，但**日志自己留一份缓存**，不复用托盘图标那份：图标只关心「变没变」，
  而日志要知道**变的是哪一半**才能决定写状态行还是电量行（见 [2.8.1](#281-行格式)），
  这个区分只有它自己需要。「最近实测」的环形缓冲同理，也是自己判断（见 [2.10.3](#2103-最近实测)）。

### 2.6 充电状态与 Tooltip

- **充电状态是四态，不是「插没插」。** `ACLineStatus` 只回答电源线在不在线，电池充满后线照样在线，
  拿它当「正在充电」的结果是满电后一直显示「正在充电」，与系统自己的托盘弹窗当场打架。
  判据改用同一个 `SYSTEM_POWER_STATUS` 里的 `BatteryFlag`：

  | 状态 | 判据 | 文案 |
  | --- | --- | --- |
  | 充电中 | `BatteryFlag & 0x08` | 正在充电 |
  | 已充满 | 无 `0x08`、`ACLineStatus == 1`、`percentage >= 100` | 电池已充满 |
  | 在线未充电 | 无 `0x08`、`ACLineStatus == 1`、`percentage < 100` | 已接通电源 |
  | 放电中 | 其余 | 使用电池 |

  - `BatteryFlag` 的 bit 3 正是系统电池弹窗读的那一位，跟着它走两边就不会各说各话；这个位 SDK
    没给名字（`BatteryFlag` 的位一个都没有），源码里自己定义常量。
  - **后两态从外部分不出来**：停在厂商充电阈值（联想/华硕那类 60%、80% 上限，或电池保养模式）上的
    电池，报的东西与真正充满的一模一样，没有任何标志位区分，只有百分比不同 —— 所以用百分比分。
  - 四个文案由 `charge_state_text()` 一处给出，tooltip 第一行、信息面板抬头（[2.10](#210-电池信息面板)）
    与电量日志（[2.8](#28-电量日志)）共用，同一个状态不给两种叫法。
...
```
  ...
  - 第一行：`<充电状态文案>：<percentage>%`（文案按上面的四态表）。
  - 第二行：`<充电功率|实时功耗>：<瓦数，一位小数> W`。
  - 第三行：`预计<充满|剩余>：<时长>`。
  - **每行一个量，一律用全角冒号分隔标签与值**：shell 用比例字体画 tooltip，空格对不出列，
    位置固定的冒号是这里唯一能做的对齐。**用全角，不用「半角冒号 + 空格」** —— 后者试过并
    退回：半角冒号在中文字体里紧贴前一个汉字，标签与值黏成一团，三行整体也显得挤。
    标签沿用信息面板（[2.10](#210-电池信息面板)）的用词，同一份数据不给两种叫法。
  - 后两行的「充电/放电」方向按**电流方向**（`Rate` 的正负）判断，不看上面的充电状态 —— 两者对能量往哪边走的判断一致，但只有 `Rate` 能给出快慢。满电或停在充电阈值上时没有电流，那时后两行整体省略。
  - **不显示「每变化 1% 用多久」**：那个数是从一个瞬时采样外推出来的，塞进悬停一闪而过的
    tooltip 里只是干扰。它归信息面板（[2.10](#210-电池信息面板)）—— 在那儿它紧挨着「最近实测」
    的真实每格时长，两个数摆在一起才有得对照。

#### 2.6.1 电量与功率的数据来源

第一行的百分比来自 `GetSystemPowerStatus`，但**后两行不能**：那里没有功率，`BatteryLifePercent`
也只有整数百分比 —— 面板要的「每变化 1% 用多久」得等一整格走完才算得出来，而且算出来的是过去时。
改从电池驱动直接取能量与功率：
...
- 换算（单位为小时）：`剩余 = Capacity / |Rate|`，`充满 = (FullChargedCapacity - Capacity) / Rate`，
  以及只有面板用的 `每变化 1% = FullChargedCapacity / 100 / |Rate|`。
- 时长格式：不足 1 分钟为 `<秒> 秒`，不足 1 小时为 `<分> 分 <秒> 秒`，否则 `<小时> 小时 <分> 分`。
  数字与单位之间留一个空格，与 UI 里其余的数字写法（`6.3 W`、`每变化 1%`、`214 次`）一致。
  **只有电量日志用不带空格的那一版**（`6分12秒`），理由见 [2.8.1](#281-行格式)；两者是同一份算术的两个出口。
  超过一小时不显示秒，秒数也**不补零** —— 这个数本来就是从一个瞬时采样外推出来的，
  秒是噪声，补零反而像在声称精确到秒。
...
**配色跟随系统主题**，沿用 [2.4](#24-主题自适应配色) 的「启动时读一次、运行期不跟随」。菜单是 user32
按系统的 Menu 主题画的，Win32 从来没给过「用哪一套」的文档化开关，唯一的入口是 uxtheme 按**序号**导出的
两个函数 —— `#135 SetPreferredAppMode` 与 `#136 FlushMenuThemes`（后者让 user32 丢掉已缓存的菜单主题）。
启动时 `LoadLibraryEx` + `GetProcAddress` 取，深色主题下调一次即可，菜单本身的构建代码不受影响。

- **限定从 System32 加载**（`LOAD_LIBRARY_SEARCH_SYSTEM32`）：按序号取的函数更要确认取自哪个文件，
  否则搜索路径上先出现的同名 DLL 就是一次 DLL 劫持。拿到的模块**不释放**（模式是 uxtheme 的进程内状态，
  且 user32 本来就为主题化持有它）。
- **任一步失败就什么都不做**，菜单退回浅色，不报错也不降级到别的画法 —— 这是未文档化接口应有的失败姿态。
- `#135` 在 build 18362 之前是 `AllowDarkModeForApp`，参数与返回都不同；本项目目标平台是
  Win10 19044+（见 [1.2](#1-硬性约束最高优先级)），落不到那个语义上，故不做版本分支。
- **传 `ForceDark` 而不是 `AllowDark`**：`AllowDark` 是让 uxtheme 自己按 `AppsUseLightTheme` 决定，
  而本程序其余部分读的是 `SystemUsesLightTheme`，这两个键用户可以设成相反 —— 那样同一个程序会同时
  显示两种主题。浅色主题下则一次都不调（uxtheme 默认就是浅色菜单），连 DLL 都不加载。
- 影响面只有本进程的菜单：程序不用任何通用控件与系统对话框。
- **不自绘菜单**（`MF_OWNERDRAW` + `MIM_BACKGROUND` 换背景画刷）：自绘只拿得到菜单项那块矩形，
  菜单窗口的边框、Win11 的圆角与阴影仍归系统画，深色内容外面会露一圈浅色边；代价是四五倍的代码
  换一个比现在更差的结果。用未文档化的开关，换来的是**整个菜单**（含边框、圆角、阴影、悬停高亮）
  与系统其它深色菜单完全一致。
...
- **写失败要容错**：若 exe 目录不可写（如放在 `Program Files`），打开/写入会失败——此时**静默禁用日志、取消菜单勾选**，绝不崩溃，也**不得**改写到别的目录作为兜底。
  写失败禁用时**要把会话状态一并清掉**（上次读数、进行中的那一格、睡眠标记），否则重新勾选后的
  第一格会带上一个横跨整个禁用期的时长，违反 [2.8.1](#281-行格式)「开启日志后的第一格不写时长」。
- **时间戳用固定格式** `yyyy-MM-dd HH:mm:ss`（本地时间），不用区域相关的本地化默认格式——固定格式稳定、可排序、跨机器一致。

#### 2.8.1 行格式

```
2026-08-26 09:55:19          80%  开始记录
2026-08-26 09:55:19          80%  使用电池
2026-08-26 10:01:31   80% -> 79%
2026-08-26 10:08:00   79% -> 78%  6分29秒
2026-08-26 10:12:03   78% -> 77%  4分3秒
2026-08-26 10:20:44          77%  已接通电源
2026-08-26 10:22:10          77%  正在充电
2026-08-26 10:28:50   77% -> 78%
2026-08-26 10:35:30   78% -> 79%  6分40秒
...
2026-08-26 11:59:02  99% -> 100%  5分12秒
2026-08-26 11:59:02         100%  电池已充满
2026-08-26 12:30:00         100%  进入睡眠
2026-08-26 20:41:12          94%  唤醒，睡眠 8小时11分，掉电 6%
2026-08-26 20:41:12          94%  使用电池
2026-08-26 20:47:30   94% -> 93%
2026-08-26 20:53:48   93% -> 92%  6分18秒
2026-08-26 22:10:05          92%  结束记录
```
...
| --- | --- | --- |
| 1 | 时间戳 | 定长 19 |
| 2 | 读数：当时的电量，或刚走完的那一格 | **右对齐**补齐到 11（`100% -> 99%` 的宽度） |
| 3 | 发生了什么：时长、状态文案、会话或睡眠标记 | 自由文本，不补齐 |

- **每行都带读数，所以读数单独成列**。这是这个版式成立的关键：需要补齐的那一列**只有 ASCII**，
  **补齐不再依赖中文字符在读者字体里有多宽**，也不需要「非 ASCII 算 2 列」那种近似；而排在它后面的第 3 列
  是最后一列，后面没有东西要对齐，于是永远不会出现为了凑宽度而补出来的大片空白。
  - 更早的一版把「状态文案」和「电量步进」塞进同一列，两者天然宽度差得远（`唤醒` 4 列、`100 -> 99%` 10 列），
    补出来的间隙在 2 到 8 格之间跳，读起来像洞不像表格。**这是被替掉的写法，不要退回去。**
- **右对齐不是左对齐**：这样每行「当前电量」的 `%` 都落在同一条竖线上，扫这一列就知道掉得快不快，
  而电量行的起点向左伸进空档里 —— 两种行共用一列却互不干扰。左对齐则要么读数错位，要么第 3 列错位。
- **时间戳每行都写全**，不用「日期只出现一次的小标题 + 每行只写时分秒」那种排法：每行自足才能 grep、能排序，
  滚动到 `.old` 时也不会有半截文件失去日期。省下的那点宽度不值这些。
- **电量行把起止都写出来**（`79% -> 78%`），不靠「上一行的读数就是起点」来省字：上一行可能是状态行、
  睡眠行，也可能因为 512KB 滚动而根本在另一个文件里 —— 每行自足是这个文件的一贯要求，和上一条时间戳同理。
  写法与面板的「最近实测」一致（[2.10.3](#2103-最近实测)），同一份数据不给两种叫法。
- **步进用半角 `->`，不用面板的 `→`**：`→` 在东亚字体里宽度是「歧义」的，可能占一格也可能占两格，
  而它正好落在唯一需要补齐的那一列里，宽度随字体变就把对齐毁了。
- **时长在日志里不加空格**：`6分12秒`、`8小时11分`，与 UI 其余各处的 `6 分 12 秒` 不同（[2.6.1](#261-电量与功率的数据来源)）。
  UI 里那个空格是把数字与单位分开，而日志里这些时长自己占一列、周围全是空白，再加空格只是把行拉长。
  实现上是同一份算术的两个出口（`format_duration` / `format_duration_compact`），不是两套换算。
  标签与值之间的空格照留（`睡眠 8小时11分`、`掉电 6%`）。
- **满电写 `100%`，不写 `FL`**：`FL` 是 [2.3](#23-托盘图标内容) 为 16px 的托盘框凑出来的缩写，
  文本文件里没有宽度限制，写缩写只会让日志需要二次解释。

**四种行**（第 3 列的四种内容）：

1. **状态行** `<四态文案>` —— 仅在充电状态**真的变化**时写（文案见 [2.6](#26-充电状态与-tooltip) 的四态表）。
   旧格式把状态贴在每一行上，结果是满屏重复的四个字，而唯一在动的那个数字淹在里面。
2. **电量行** `<这一格的时长>`（读数列写 `<from>% -> <to>%`）—— 百分比每变化一格写一行，
   时长是这一格实际走完的墙钟时间。
   **这是旧格式最大的缺失**：它只有时间戳，「这一格用了多久」要读的人自己拿两行相减，而程序本来就知道。
3. **会话行** `开始记录` / `结束记录`。勾选开启时写 `开始记录`，紧跟一行状态行交代起点；
   取消勾选与**程序退出**都写 `结束记录`，这样文件里每一段空白都有名字，不会把「退出了」看成「睡着了」。
   `开始记录` 前空一行分隔不同的会话，但文件为空时不写，免得新文件开头就是空行。
4. **睡眠行** `进入睡眠` 与 `唤醒，睡眠 <时长>，掉电 <n>%` —— 见 [2.8.2](#282-睡眠与唤醒)。

**电量与状态同时变化时**（充到 100% 那一刻、插上电源的同时又跳了一格）**先写电量行再写状态行**：
前者是后者的原因，而且这样一来「文件里最后一条状态行」永远描述当前状态，往回翻不必担心它被一条电量行盖过去。

**时长在这三种情况下整个第 3 列留空**（不写 `--` 之类占位，与 [2.10.2](#2102-拿不到的字段整行不画) 同一取向），
那一行就只剩时间戳与那一格的起止，说明「这一格确实走完了，但走了多久无从谈起」：开启日志后的第一格、
跨充电状态变化的那一格、跨睡眠的那一格。三者的共同点是这一格的起点不是一次真正的百分比跳变，
从那个时刻算出来的数会偏小 —— 只有百分比跳变才是合法的起算点。

#### 2.8.2 睡眠与唤醒

收到 `PBT_APMSUSPEND` 写 `进入睡眠`，收到 `PBT_APMRESUMEAUTOMATIC` 写 `唤醒`，后者补上睡了多久、
电量变化了多少。

- **这是日志相对信息面板不可替代的地方**：面板的「最近实测」遇到睡眠会 `discard_pending()` 把进行中的那格
  直接丢掉（[2.10.3](#2103-最近实测)），合盖 8 小时这件事在面板里不留任何痕迹。而日志里那一段空白如果没有名字，
  读的人只会以为电量 8 小时没动。
- **只处理 `PBT_APMRESUMEAUTOMATIC`**：它每次唤醒都会发；`PBT_APMRESUMESUSPEND` 只在有人主动唤醒时额外发一次，
  两个都接会让一半的唤醒写两遍。
- 时长用 `GetTickCount64` 的差值：它在睡眠期间继续走（`QueryUnbiasedInterruptTime` 不走），而这里要的正是墙钟。
- 电量变化按方向写 `掉电 <n>%` / `充电 <n>%`（睡在充电器上）/ `电量未变`。
- **唤醒后要把「上次百分比」重新对齐到唤醒时的读数**，否则接下来的第一次刷新会写出 `100% -> 94%`
  这种跨越 6 格的假步进 —— 那 6% 已经由唤醒行交代过了。睡眠期间充电状态若变了则不对齐，
  留给状态行照常报告。
...
托盘图标**单击**弹出的一个小面板，装 tooltip 装不下的东西：tooltip 只有 128 个宽字符、只能显示纯文本、
...
功耗 / 每变化 1% / 预计剩余的算法都在 [2.6.1](#261-电量与功率的数据来源)（tooltip 只用其中两项，
「每变化 1%」是面板独有的），只是显示的功率取**最近 3 次采样的滑动平均** —— 瞬时功率抖动大，2 秒跳一个数看着会乱，
...
- **不要拿 `MSAcpi_ThermalZoneTemperature` 顶替**：那是 CPU / 主板热区不是电池温度，数值差得远，
  而且要走 WMI。这条路走不通，别再试。
- **滑动平均只在同一个方向上取**：电流方向翻转（面板开着插上充电器）时窗口清空重来，否则 -11 W 与
  +30 W 会平均成一个从未出现过的读数，正负恰好抵消时整个功率区连分隔线一起消失又出现。
  速率为 0 不算一个方向（它既可能是真的没有电流，也可能是驱动报的 `BATTERY_UNKNOWN_RATE`），
  只有真正的反向才重置。
- **静态与动态分开**：循环次数、设计容量、出厂日期在面板打开时读一次；只有功率、剩余容量、温度进每 2 秒的
  重采路径。**静态字段随句柄一起作废**：丢句柄（热拔、IOCTL 失败）或关面板时和动态字段一样清零，
  不能只清一半，留下一行上一块电池的出厂日期。
...
- 一整段的字段全都拿不到时，**连同那段的分隔线一起省掉**，别留一条孤零零的横线。
- 因此**面板高度是动态的**：先在布局阶段算出实际要画多少行，再据此定窗口大小与弹出位置，
  不能把高度写死在窗口创建时。宽度相反，**按最长的中文标签定死**（随 DPI 缩放）。
- 例外是「最近实测」：列表为空时显示「暂无数据」占位而不是整段消失 —— 空是关于本次开机的事实，
  不是关于这台机器的，它会随着运行自己填上。

#### 2.10.3 最近实测

内存里一个定长环形缓冲，每次电量百分比变化时压一条（变化前后的百分比 + 实际耗时），面板显示最近六条。
...
- **这几行不缩进**，与面板其它行起在同一条左边线上。分段的分隔线、「最近实测」这个标题、
  以及这几行整体更浅的颜色，已经把它们与上下两段分开了三遍；再加一层缩进只会打断面板唯一
  一条干净的左边线，看着像排版事故。左列是数据（`76 → 75%`）不是标签，对齐到同一条线上
  也不会被误读成又一组「标签 / 值」。

#### 2.10.4 窗口与交互

- **触发**：托盘图标左键**单击**（`NOTIFYICON_VERSION_4` 下是 `NIN_SELECT`，键盘那一路是 `NIN_KEYSELECT`），
  再次单击关闭。跟系统自己的电量图标一致 —— 那个也是单击就弹。
- **形态**：`WS_POPUP | WS_EX_TOOLWINDOW | WS_EX_TOPMOST`，无标题栏，圆角靠 `SetWindowRgn` 裁形状 +
  自绘边框。不进任务栏、不进 Alt+Tab。
  - 窗口是**首次单击时才创建**的，而 `CreateWindowEx` 万一失败下次单击还会再来一遍，
    那时 `RegisterClassEx` 会以 `ERROR_CLASS_ALREADY_EXISTS` 失败。**这个错误码要当成成功**
    （或用一次性标志只注册一次），否则一次偶发失败就让面板在本进程内再也起不来。
...
- **定位**：以托盘图标为锚点，跟 Windows 自己的音量 / 网络 / 电量面板同一种行为 —— 紧贴任务栏，
  ...
  - **平行于任务栏的轴**：面板与图标**居中对齐**（任务栏在下时，面板横向中线对准图标中线），
    然后钳制进工作区 —— 图标离屏幕边缘太近时整体往里推，不允许有任何一部分落在工作区外。
    通知区一般都靠着屏幕角落，所以钳制经常会生效，那时看到的就是贴边而不是正对图标；这是对的，
    保证不出界优先。
...
- **失焦即隐藏**：`WM_ACTIVATE` 收到 `WA_INACTIVE` 就 `SW_HIDE`，跟 Windows 自己的电量面板一致。
  ...
  - 点托盘图标本身会先让面板失焦而隐藏，等这一下的消息到达时它已经关了 —— 不做处理的话「点开着的面板」
    会关掉再立刻开回来，看着像什么都没发生。所以隐藏后极短的时间窗内收到的单击一律当关闭。
- **DPI**：处理 `WM_DPICHANGED`，重算字体与布局（忽略系统建议的矩形，位置本来就要按锚点重算）。
  托盘图标那套 90% / 超采样常量与这里无关。
...
- **配色**：背景 / 文字 / 次要文字 / 分隔线 / 边框各两套（深浅），沿用 [2.4](#24-主题自适应配色) 的
  「启动时读一次、运行期不跟随」。
- **绘制**：纯 GDI 手绘一个 `WM_PAINT`，**不用任何控件**。整个面板就是若干行「左标签 + 右数值」，
  用 `DrawTextW` 分两次对齐画。经内存位图再 `BitBlt` 上屏 —— 每个 tick 都整面重画，直接在屏幕上填背景
  再压字会闪。
...
1. 完整可编译的 C++ 源码，按职责拆成少量文件，保持简单。当前划分：
   ...
   | `src/info_panel.cpp` | 单击弹出的电池信息面板：布局、定位与绘制 |
...
- 三个菜单项（电量日志、开机启动、退出）行为正确，勾选状态与实际状态一致；深色主题下菜单整体为深色
  （背景、文字、勾选标记、悬停高亮与边框都是），浅色主题下与从前无异。
- 单击弹出的信息面板贴着任务栏出现（四种任务栏位置、图标折叠进溢出区时都不跑出工作区），失焦 / Esc /
  再次单击都能关掉，关掉后进程回到空闲态无定时器；拿不到的字段整行不出现而不是显示占位符。
- 日志的电量行带得出这一格的时长，状态只在变化时占一行；合盖睡一觉再唤醒，文件里有对应的两行，
  且不会写出跨越好几格的假步进。
