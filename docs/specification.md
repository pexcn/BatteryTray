...
```
2026-08-26 09:55:19  开始记录
2026-08-26 09:55:19  使用电池    80%
2026-08-26 10:01:31  80 -> 79%   6 分 12 秒
2026-08-26 10:08:00  79 -> 78%   6 分 29 秒
2026-08-26 10:12:03  78 -> 77%   4 分 3 秒
2026-08-26 10:20:44  已接通电源  77%
2026-08-26 10:22:10  正在充电    77%
2026-08-26 10:28:50  77 -> 78%   6 分 40 秒
2026-08-26 11:59:02  99 -> 100%  5 分 12 秒
2026-08-26 11:59:02  电池已充满  100%
2026-08-26 12:30:00  进入睡眠    100%
2026-08-26 20:41:12  唤醒        94%   睡眠 8 小时 11 分，掉电 6%
2026-08-26 20:41:12  使用电池    94%
2026-08-26 20:47:30  94 -> 93%   6 分 18 秒
2026-08-26 22:10:05  结束记录
```

**每行一个事件，分三到四列，列间两个空格**，前面的列按显示宽度补空格补齐：

| 列 | 内容 | 宽度 |
| --- | --- | --- |
| 1 | 时间戳 | 定长 19 |
| 2 | 事件：状态文案、电量步进或会话标记 | 补齐到 10 显示列 |
| 3 | 百分比（事件行）或这一格的时长（电量行） | 仅第 4 列存在时补齐到 4 |
| 4 | 补充说明，目前只有唤醒行有 | 不补 |

- **时间戳每行都写全**，不用「日期只出现一次的小标题 + 每行只写时分秒」那种排法：每行自足才能 grep、能排序，
  滚动到 `.old` 时也不会有半截文件失去日期。省下的那点宽度不值这些。
- **第 2 列宽度取 10 不是凑的**：里面只会出现两种东西，而两者最宽处正好相等 —— 四态文案最长的
  `电池已充满` 是 5 个全角字符，电量步进最长的 `100 -> 99%` 是 10 个半角字符。
- **补齐按显示宽度算，非 ASCII 一律算 2 列**。这一列只会出现中文文案与半角的步进文本，没有需要真正宽度表的字符。
  等宽字体下列是齐的（记事本默认在 Win10 是 Consolas、Win11 是 Cascadia Mono，中文走全角回退）；
  换成比例字体就对不齐，但那种情况下任何方案都对不齐 —— 与 [2.6](#26-充电状态与-tooltip) 里 tooltip
  放弃对齐是同一个道理，区别是文本文件的默认字体是等宽的，所以这里补空格划算。
- **步进用半角 `->`，不用面板的 `→`**：`→` 在东亚字体里宽度是「歧义」的，可能占一格也可能占两格，
  一列宽度随字体变就把上面的补齐全毁了。其余写法（`79 -> 78%`、时长格式）与面板「最近实测」一致，
  同一份数据不给两种叫法。
...
1. **状态行** `<四态文案>  <percentage>%` —— 仅在充电状态**真的变化**时写（文案见
   [2.6](#26-充电状态与-tooltip) 的四态表）。旧格式把状态贴在每一行上，结果是满屏重复的四个字，
   而唯一在动的那个数字淹在里面。
2. **电量行** `<from> -> <to>%  <这一格的时长>` —— 百分比每变化一格写一行。时长是这一格实际走完的墙钟时间，
   格式用 [2.6.1](#261-电量与功率的数据来源) 的时长格式。**这是旧格式最大的缺失**：它只有时间戳，
   「这一格用了多久」要读的人自己拿两行相减，而程序本来就知道。
3. **会话行** `开始记录` / `结束记录` —— 只占第 2 列。勾选开启时写 `开始记录`，紧跟一行状态行交代起点；
   取消勾选与**程序退出**都写 `结束记录`，这样文件里每一段空白都有名字，不会把「退出了」看成「睡着了」。
   `开始记录` 前空一行分隔不同的会话，但文件为空时不写，免得新文件开头就是空行。
4. **睡眠行** `进入睡眠  <percentage>%` 与 `唤醒  <percentage>%  睡眠 <时长>，掉电 <n>%` ——
   见 [2.8.2](#282-睡眠与唤醒)。

**两者同时变化时**（充到 100% 那一刻、插上电源的同时又跳了一格）**先写电量行再写状态行**：前者是后者的原因，
而且这样一来「文件里最后一条状态行」永远描述当前状态，往回翻不必担心它被一条电量行盖过去。

**时长在这三种情况下整列留空**（不写 `--` 之类占位，与 [2.10.2](#2102-拿不到的字段整行不画) 同一取向）：
开启日志后的第一格、跨充电状态变化的那一格、跨睡眠的那一格。三者的共同点是这一格的起点不是一次真正的
百分比跳变，从那个时刻算出来的数会偏小 —— 只有百分比跳变才是合法的起算点。
...
- **唤醒后要把「上次百分比」重新对齐到唤醒时的读数**，否则接下来的第一次刷新会写出 `100 -> 94%` 这种
  跨越 6 格的假步进 —— 那 6% 已经由唤醒行交代过了。睡眠期间充电状态若变了则不对齐，留给状态行照常报告。
