...
- Tooltip 文案（`NOTIFYICONDATA.szTip`，中文）**共四行**，用 `\r\n` 分隔：
...
```
  ...
  - **每行一个量，一律用全角冒号分隔标签与值**：shell 用比例字体画 tooltip，空格对不出列，
    ...
    退回：半角冒号在中文字体里紧贴前一个汉字，标签与值黏成一团，四行整体也显得挤。
    标签沿用信息面板（[2.10](#210-电池信息面板)）的用词，同一份数据不给两种叫法。
  - 后三行的「充电/放电」方向按**电流方向**（`Rate` 的正负）判断，不按 `ACLineStatus` —— 满电插着电源时线是在线的，但没有电流，那时后三行整体省略。

#### 2.6.1 电量与功率的数据来源

第一行的百分比来自 `GetSystemPowerStatus`，但**后三行不能**：`BatteryLifePercent` 只有整数百分比，
...
- 时长格式：不足 1 分钟为 `<秒> 秒`，不足 1 小时为 `<分> 分 <秒> 秒`，否则 `<小时> 小时 <分> 分`。
  数字与单位之间留一个空格，与 UI 里其余的数字写法（`6.3 W`、`每 1%`、`214 次`）一致。
