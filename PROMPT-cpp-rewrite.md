# 提示词：用 C++ / Win32 重写 percentage

把下面整段作为提示词交给 AI。它描述了一个把「Windows 系统托盘电量百分比显示器」从 C#/.NET Framework 重写为原生 C++/Win32 的完整任务，包含现有行为规格与新增需求。

---

## 角色与目标

你是资深 Windows C++ 工程师。请用**原生 C++（C++23）+ Win32 API** 重写一个名为 `percentage` 的系统托盘小程序。原程序用 C#/.NET Framework 4.8.1 + WinForms 编写，功能是在任务栏通知区（托盘）用一个动态生成的图标显示当前电池百分比。

**重点：要复刻的是「功能」，不是逐行照搬原程序的实现方式。** 下文的「现有行为规格」描述的是**功能需求与用户可见效果**，不是让你机械翻译 C# 代码。凡是原生 Win32 有更优、更省资源、更健壮的做法，请主动采用并简要说明理由（例如用电源事件通知替代轮询、用纯 GDI 替代 GDI+、更高效的图标缓存策略等）。目标是产出一个功能等价、但**零第三方依赖、极低 CPU/内存占用、去除调试信息**的原生实现。

## 硬性约束（最高优先级）

1. **性能优先**：低 CPU、低内存占用。空闲时几乎不占 CPU；常驻内存尽量小。
   - 用消息/事件驱动，**不要轮询**：通过 `RegisterPowerSettingNotification` + `WM_POWERBROADCAST` 由系统推送电量/充电变化，收到事件才刷新（详见下方「刷新机制」）。
...
2. **编译与体积**：产物要经过优化并去除调试信息（相当于 GCC 的 `-O3` + `strip`）。
   - **工具链：MSVC（Visual Studio / Build Tools）。** 在「零第三方依赖 + 小体积」目标下，MSVC 配静态 CRT 能做到真正无 DLL 依赖且体积最小。
     ...
     - `/MT` 静态链接 CRT，使 `.exe` 不依赖任何 VC++ 运行时可再发行包；给出完整可复制的 `cl` + `link`（或单条 `cl`）命令。
     - 说明如何嵌入 manifest 与图标资源（`rc.exe` 编译 `.rc`，或用 `/MANIFEST` / `mt.exe`）。
   - 可追加体积优化说明（`/OPT:REF,ICF` 已做冗余消除，如仍需进一步压缩可说明取舍）。
   - 不链接系统库以外的任何东西；不要求 vcpkg/conan 等包管理器。
3. **零第三方依赖**：只允许使用 Windows 自带的系统库（user32、gdi32、shell32、advapi32、gdiplus 等）与 C++ 标准库。**禁止**引入任何外部开源库、框架或 NuGet/vcpkg 包。
   - 文字绘制优先考虑纯 GDI（`CreateFont` + `TextOut` + `GetTextExtentPoint32`）以避免依赖 GDI+；若为字形质量选用 GDI+，需说明理由并确保 `gdiplus.dll` 是系统自带、无需分发。
4. **单文件可执行**：产物是一个独立的 `percentage.exe`，双击即用，无需安装、无需额外 DLL 分发。
5. **绿色便携**：程序自身不产生配置文件，不向用户目录/系统目录写入任何东西。唯一会写的文件是「电量日志」，且**只写在 exe 所在目录**（见日志功能）。唯一的持久化状态是「开机启动」项写在注册表 `Run` 键（读写注册表不破坏便携性，是 Windows 开机启动的标准做法）；日志开关等其它状态不持久化。

## 需要复刻的现有行为（功能规格）

- **进程形态**：无主窗口的托盘常驻程序。创建一个**隐藏的普通顶层窗口**（正常 `CreateWindowEx` 创建，但从不 `ShowWindow`、不带 `WS_VISIBLE`）来接收托盘回调与系统消息，进程通过消息循环常驻。
  ...
  - **不要用 message-only 窗口（`HWND_MESSAGE`）**：它收不到广播消息，而本程序依赖两个广播——`PBT_APMPOWERSTATUSCHANGE`（充电插拔）与 `TaskbarCreated`（explorer 重启后恢复图标）。用隐藏顶层窗口才能同时收到定向消息与广播消息。
- **刷新机制（事件驱动，禁止轮询）**：**不要用定时器周期性轮询**。改为向系统注册电源事件通知，由系统主动推送变化：
...
- **托盘图标内容**：把电量数字**渲染成文字位图**再转成 `HICON` 设为托盘图标（不是用现成图标资源）。
  - 字体：默认用系统 UI 字体（`SystemParametersInfo(SPI_GETNONCLIENTMETRICS)` 取 `lfMessageFont`，回退到 `Segoe UI`）。内容只有数字与 `FL`，不要用 CJK 字体（如 Microsoft YaHei）——对纯 ASCII 无收益且非核心系统字体。
  - **字号不要写死**：托盘图标实际边长是 `GetSystemMetrics(SM_CXSMICON)`，随 DPI 缩放（100%→16px、150%→24px、200%→32px）。按当前 DPI 求出图标尺寸后，把字号**适配到该尺寸**（让最宽内容如两位数/`FL` 刚好填满、不被裁切），与「DPI 感知」要求一致。
  - 电量 = `round(BatteryLifePercent * 100)`。**当电量 > 99 时显示为 `FL`**（表示满电 Full），否则显示整数百分比。
  - 文字在图标位图内**居中**（不要写死 2px 偏移那种按固定尺寸调出来的经验值，随字号/DPI 动态居中）。
  - **抗锯齿用灰度而非 ClearType**：图标背景透明（alpha 通道），而 ClearType 是亚像素渲染、假设背景不透明，用在透明位图上会在字符边缘产生彩色毛边。请用 `ANTIALIASED_QUALITY`（灰度抗锯齿），边缘干净且与任意任务栏背景兼容。
  - 背景透明（图标 alpha 通道透明）。
- **主题自适应配色**：读取注册表
  `HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Themes\Personalize` 的 `SystemUsesLightTheme`：
  值为 1（浅色主题）用**黑色**文字，否则（深色/缺失）用**白色**文字。
  - 原程序只在启动时读一次。**保持一致：仅在启动时读取一次配色，运行期不跟随主题切换。**
- **变化去重**：电源事件可能短时间内重复触发。仅当 `(percentage, isCharging)` 相对上次**确有变化**时才重建图标与写日志（沿用缓存判断作为一层防抖），避免无谓的图标重建。
- **充电状态**：`isCharging = (电源线在线)`，通过 `GetSystemPowerStatus` 的 `ACLineStatus == 1` 判断。
- **Tooltip 文案**（`NOTIFYICONDATA.szTip`，中文）：
  - 充电中：`正在充电：<percentage>%`
  - 用电池：`使用电池：<percentage>%`
- **右键上下文菜单**（`TrackPopupMenu`），当前含两项，你需要在此基础上**新增一项**（见下）：
  1. `电量日志`（可勾选，见下）
  2. `退出`：隐藏并移除托盘图标（`Shell_NotifyIcon` + `NIM_DELETE`），退出消息循环。
- **电量日志功能**（勾选式开关）：
  ...
  - **时间戳用固定格式** `yyyy-MM-dd HH:mm:ss`（本地时间），不要用区域相关的本地化默认格式——固定格式稳定、可排序、跨机器一致。

## 新增需求

1. **性能优先**：见硬性约束 1，作为整体设计基调贯穿始终。
2. **编译剥离调试信息**：见硬性约束 2，交付时给出可复制粘贴的构建命令。
3. **零第三方依赖**：见硬性约束 3。
4. **「开机启动」托盘菜单项**（勾选式）：
   - 在右键菜单新增一项 `开机启动`，用勾选状态（`MF_CHECKED`/`MF_UNCHECKED`）表示当前是否已启用。
   - 实现方式：写入/删除注册表运行项
     `HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run`，值名用程序名（如 `percentage`），值数据为当前可执行文件的完整路径（用 `GetModuleFileName` 获取，路径含空格时加引号）。
...
- 所有 GDI 对象（`HFONT`、`HBITMAP`、`HDC`、`HICON`）与内核句柄（文件、注册表 key）都要成对释放；建议用 RAII 包装（自定义 `unique_handle` 类模板或 `std::unique_ptr` 配合自定义 deleter），禁止裸露的手动释放散落各处。
- 注册表读取要做空值/类型/长度校验，键或值缺失时走安全默认分支，不得崩溃。
- 单例：可选实现（用命名 mutex 防止重复启动），若实现请作为可选项说明。
